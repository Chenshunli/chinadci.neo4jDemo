...
public class GraphDbController {
    ...
    @ApiOperation(value = "根据标签名称和节点id获取具体节点信息")
    @RequestMapping(value = "/getData", method = RequestMethod.GET)
    public ServiceResult getNodes(
            @RequestParam(name = "labelName",required = false) @ApiParam(value = "查询标签") String label,
            @RequestParam(name = "id", required = false) @ApiParam(value = "id") List<Long> ids){
//            return graphDbService.findNodes(ids);
        return new ServiceResult(graphDbService.findNodes(label,ids));
    }

    @ApiOperation(value = "查询某一类标签下所有节点")
    @RequestMapping(value = "/getNodes", method = RequestMethod.GET)
    public ServiceResult getAllData(
            @RequestParam(name = "labelName",required = false) @ApiParam(value= "查询标签") String labelName
    ){
        if(labelName==null){
            labelName = "pddk";
        }
        return graphDbService.getData(labelName);
    }

    @ApiOperation(value = "图数据关系构建")
    ...
    public ServiceResult createRelationShip(
            ...
            @RequestParam(name = "endNodeLabel") @ApiParam(value = "结束节点标签",required = true) String label2,
            @RequestParam(name = "relationLabel") @ApiParam(value = "关系标签",required = true) String relationLabel,
            @RequestParam(name = "relationTable") @ApiParam(value = "关系表名称",required = true) String relationTable
    ){
